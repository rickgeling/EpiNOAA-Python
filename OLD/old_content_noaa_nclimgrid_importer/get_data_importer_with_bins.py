...
def fetch_and_clean_daily_weather_batched( 
    ...
    ### NEW
    print("  Sorting data by FIPS and date to prepare for tmin_next_day calculation...")
    df_daily_weather = df_daily_weather.sort(["fips", "date"])

    # Now, create the 'tmin_next_day' column.
    # The .shift(-1) gets the value from the next row.
    # .over("fips") is crucial: it treats this as a window function, so the shift
    # operation resets for each new FIPS code. This prevents the Tmin from one
    # county leaking into the last day of the previous county.
    print("  Calculating tmin_next_day...")
    df_daily_weather = df_daily_weather.with_columns(
        pl.col("tmin").shift(-1).over("fips").alias("tmin_next_day")
    )    
    ## END NEW
    ...
    # === NEW ===
    if "tmin_next_day" not in final_columns_to_keep:
        final_columns_to_keep.append("tmin_next_day")             
    # NEW         
...
# =============================================================================
# NEW: TEMPERATURE BINNING HELPER FUNCTIONS
# =============================================================================
def get_daytime_temp(hour: float, tmin: float, tmax: float) -> float:
    """
    Calculates temperature during the daytime heating phase using a sine curve.
    Assumes Tmin at hour 6 and Tmax at hour 15.
    """
    day_duration = 9
    return tmin + (tmax - tmin) * np.sin(np.pi * (hour - 6) / day_duration)

def get_nighttime_temp(hour: float, tmax: float, tmin_next_day: float) -> float:
    """
    Calculates temperature during the nighttime cooling phase using a cosine curve.
    This final version uses the most robust hour adjustment logic.
    """
    night_duration = 15  # From 15:00 today to 6:00 next day (a 15-hour period)

    # This is the clearest and most robust way to handle the hour adjustment.
    # It correctly identifies that only hours before 6 AM belong to the next day's cycle.
    h_adjusted = hour + 24 if hour < 6 else hour

    # The rest of the formula remains the same, as it is mathematically correct.
    amplitude = (tmax - tmin_next_day) / 2
    vertical_shift = (tmax + tmin_next_day) / 2

    return vertical_shift + amplitude * np.cos(np.pi * (h_adjusted - 15) / night_duration)

def calculate_daily_exposure_bins(row: Tuple) -> Dict[str, float]:
    """
    Calculates the time (in hours) spent in each 1-degree temperature bin for a single day.
    Uses parameters from the config file. Expects row tuple: (tmin, tmax, tmin_next_day)
    """
    tmin, tmax, tmin_next_day = row
    
    if tmin is None or tmax is None or tmin_next_day is None:
        return {}

    bins = np.arange(config.TEMP_BINS_MIN, config.TEMP_BINS_MAX + 1)
    exposure_hours = {f"bin_{b}": 0.0 for b in bins}
    
    interval_minutes = config.TEMP_BINS_INTERVAL_MINUTES
    hours_per_interval = interval_minutes / 60.0
    
    for i in range(int(24 / hours_per_interval)):
        hour_float = i * hours_per_interval
        temp = get_daytime_temp(hour_float, tmin, tmax) if 6 <= hour_float < 15 else get_nighttime_temp(hour_float, tmax, tmin_next_day)
        temp_bin = int(np.floor(temp))
        bin_col_name = f"bin_{temp_bin}"
        
        if bin_col_name in exposure_hours:
            exposure_hours[bin_col_name] += hours_per_interval
            
    return exposure_hours
...
def _calculate_seasonal_aggregates(df_season: pl.DataFrame, season_prefix: str) -> Dict[str, Any]:
    ...
    # --- Original GDD, KDD, PREC, CHD Logic (Unchanged) ---
    ...
    # === START: Final, Corrected Bin Calculation Logic ===
    binning_required_cols = ["tmin", "tmax", "tmin_next_day"]
    binning_cols_present = all(col in df_season_with_daily_calcs.columns for col in binning_required_cols)
    
    if binning_cols_present:
        has_any_nulls = df_season_with_daily_calcs.select([pl.col(c).is_null().any() for c in binning_required_cols]).row(0)
        season_has_missing_bin_data = any(has_any_nulls)
    else:
        season_has_missing_bin_data = True

    all_bin_names_for_season = [f"bin_{b}_{season_prefix}" for b in range(config.TEMP_BINS_MIN, config.TEMP_BINS_MAX + 1)]
    
    if season_has_missing_bin_data:
        for bin_col_name in all_bin_names_for_season:
            aggs[bin_col_name] = np.nan
    else:
        # Define the exact schema that our function will return
        bin_schema = {f"bin_{b}": pl.Float64 for b in range(config.TEMP_BINS_MIN, config.TEMP_BINS_MAX + 1)}

        # Use the more robust struct.apply method instead of map_rows
        # A lambda function is used to unpack the struct's fields for our existing function
        bins_struct = df_season_with_daily_calcs.with_columns(
            pl.struct(binning_required_cols)
            .apply(lambda x: calculate_daily_exposure_bins((x['tmin'], x['tmax'], x['tmin_next_day'])), return_dtype=pl.Struct(bin_schema))
            .alias("bins")
        )
        
        # Unnest the struct and sum the results to get seasonal totals
        seasonal_bin_aggs = bins_struct.select("bins").unnest("bins").sum()
        
        # Add the results to our main aggregations dictionary
        for col_name in seasonal_bin_aggs.columns:
            aggs[f"{col_name}_{season_prefix}"] = seasonal_bin_aggs.get_column(col_name).item()
        
        # Ensure all possible bin columns are present, filling with 0 if a bin was never entered
        for bin_col_name in all_bin_names_for_season:
            if bin_col_name not in aggs:
                aggs[bin_col_name] = 0.0
    
    # === END: Final, Corrected Bin Calculation Logic ===
...
def main():
    ...
    path_df = "../02_weather_climdiv_crosswalk/created_dfs_step2/"
    input_csv_filename = "df_yield_climdiv_2025.csv" 
    input_csv_path = os.path.join(path_df, input_csv_filename)
    
    output_csv_path = "created_dfs_step_final/df_final_importer_2025_bins.csv"
        ...
        if cleaned_df_for_state_batch is None or cleaned_df_for_state_batch.height == 0:
            ...
            for fips_code in fips_in_state_list:
                ...
                if df_single_fips_daily_data.height == 0 and relevant_yield_years_for_fips:
                    ...
                    for year_to_aggregate in relevant_yield_years_for_fips:
                        print(f"--> AGGREGATING FOR FIPS: {fips_code}, YEAR: {year_to_aggregate}")
    ...
    if not all_yearly_weather_aggregates:
        ...
        # Dynamically add the new temperature bin columns to the schema
        for prefix in ["GS", "SGF"]:
            for temp_bin in range(config.TEMP_BINS_MIN, config.TEMP_BINS_MAX + 1):
                aggs_schema[f"bin_{temp_bin}_{prefix}"] = pl.Float64
