...
import sys
#add repo root to Python path (resolved from this file, not the current working
...
#--- Configuration: which crop to run this script for ---
#same role as the config cell in
#02_weather_climdiv_crosswalk/merge_yield_monthly_weather.ipynb: one place that
#sets the crop and the matching input/output filenames.
...
output_csv_filename = "df_compare_months_local_corn_paper.csv"      #"df_compare_months_local_soy_paper.csv"

#safety check: make sure crop and both filenames actually agree, so a
#half-finished comment-toggle (only one of the two lines swapped) gets caught
#immediately instead of silently producing e.g. a "corn" output built from soy data.
other_crop = "soy" if crop == "corn" else "corn"
for _label, _value in [("input_csv_filename", input_csv_filename),
                       ("output_csv_filename", output_csv_filename)]:
    if other_crop in _value.lower():
        raise ValueError(
            f"crop = '{crop}' but {_label} = '{_value}' contains '{other_crop}' "
            f"— looks like only some of the corn/soy lines got toggled."
        )
    if crop not in _value.lower():
        raise ValueError(
            f"crop = '{crop}' but {_label} = '{_value}' does not contain '{crop}' "
            f"— looks like only some of the corn/soy lines got toggled."
        )
...
def load_and_clean_local_daily_weather(
    ...
    for year_val in range(start_year_to_scan, end_year_to_scan + 1):
        ...
        if os.path.isdir(year_folder):
            ...
            for month_num in range(1,13): #log this for each month for consistancy if needed downstream
...
def main():
    ...
    ##----- GET THE DATA ------
    #crop / input_csv_filename / output_csv_filename are set in the config block
    #at the top of this file. Both paths are built from script_dir so the script
    #behaves the same regardless of which directory it is launched from.
    ...
    output_csv_path = os.path.join(script_dir, "created_dfs_monthly_comparison", output_csv_filename)
    ...
    #local weather data year folders live in extracted_noaa_nclimgrid_data_local/
    local_data_base_path = os.path.join(script_dir, "extracted_noaa_nclimgrid_data_local")
    ...
    #loud upfront warning about missing year folders. The local method can only
    #aggregate years that were actually downloaded; without this the run still
    # "succeeds" and writes NaN weather for those years, which is easy to miss.
    missing_year_folders = [
        y for y in range(scan_start_year, scan_end_year + 1)
        if not os.path.isdir(os.path.join(local_data_base_path, str(y)))
    ]
    if missing_year_folders:
        print(f"\n*** WARNING: no local Parquet folder for {len(missing_year_folders)} year(s) in "
              f"range {scan_start_year}-{scan_end_year}: {missing_year_folders}")
        print( "***          Every monthly aggregate for those years will be NaN.")
        print(f"***          Download the missing daily data into {local_data_base_path} for a complete run.")
