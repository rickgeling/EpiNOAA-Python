...
# Which crop to compare
crop = "corn"  # "corn" or "soy"

# Stage-3 outputs to compare: 03a (local Parquet) vs 03b (S3 importer).
#
# Deliberately the BASE importer output, not the precip-extremes variant: the two
# are AST-identical for the five metrics compared below (verified 2026-08-17), so
# comparing like for like keeps any difference attributable to the data source
# rather than to the metric set.
#
# Both filenames are derived from `crop` rather than being comment-toggled the way
# stages 01/02/03a do it. Those stages need the toggle because the crop also
# changes the folder; here it doesn't, so deriving the names makes it impossible
# for the two to disagree and no safety check is needed.
filename_local = f"df_final_local_{crop}_paper.csv"
filename_importer = f"df_final_importer_{crop}_paper.csv"

path_local = os.path.join(REPO_ROOT, "03a_weather_nclimgrid_local", "created_dfs_step_final")
path_importer = os.path.join(REPO_ROOT, "03b_weather_nclimgrid_importer", "created_dfs_step_final")

LOCAL_CSV_PATH = os.path.join(path_local, filename_local)
S3_CSV_PATH = os.path.join(path_importer, filename_importer)


def _report_missing(label, missing_path):
    """Explain a missing input instead of just naming the path that failed.

    The previous version pointed at 'df_final_importer.csv', which never existed
    anywhere in the repo, and the bare 'file not found' gave no hint of that.
    """
    folder = os.path.dirname(missing_path)
    print(f"Error: {label} CSV file not found at '{missing_path}'")
    if os.path.isdir(folder):
        available = sorted(f for f in os.listdir(folder) if f.endswith(".csv"))
        print(f"  Files present in {folder}:")
        for f in available or ["  (none)"]:
            print(f"    - {f}")
        print(f"  Either set crop= to match one of these, or re-run the stage-3 script that produces it.")
    else:
        print(f"  The folder '{folder}' does not exist yet.")
...
def compare_weather_csvs():
    ...
    print(f"  importer : {S3_CSV_PATH}")

    # --- Load Data ---
    if not os.path.exists(LOCAL_CSV_PATH):
        _report_missing("Local", LOCAL_CSV_PATH)
        return
    if not os.path.exists(S3_CSV_PATH):
        _report_missing("S3/importer", S3_CSV_PATH)
        return
